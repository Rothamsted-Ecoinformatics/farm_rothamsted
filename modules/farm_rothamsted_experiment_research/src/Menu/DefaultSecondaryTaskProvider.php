...
namespace Drupal\farm_rothamsted_experiment_research\Menu;
...
/**
 * Provides default entity tasks as secondary tasks.
 */
class DefaultSecondaryTaskProvider implements EntityLocalTaskProviderInterface, EntityHandlerInterface {
    ...
    if (in_array('canonical', $link_templates) && count($link_templates) > 1) {
      ...
      foreach ($link_templates as $rel) {
        ...
        if ($rel == 'canonical') {
          ...
          $tasks["$entity_type_id.canonical.secondary"] = [
            'title' => $this->t('View'),
            'route_name' => $route_name,
            'parent_id' => $parent,
            'weight' => $weight,
